<template>
  <div>
    <my-header title="stupid bird" :back="true"></my-header>
    <div class="m-content">
      <canvas id="stupidbird" class="stupid-bird"></canvas>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    init() {
      this.canvas = document.getElementById('stupidbird');
    ...
    collision() { // 碰撞检测，判断鸟与第一根柱子是否碰撞（鸟的体积小于2根柱子之间的距离，所以只用检测第一根柱子）
      ...
      if ((this.bird.y < y) || ((this.bird.y + 50) > (y + this.empty))) {
        this.gameOver = true;
        this.$msg({
          message: `游戏结束，总得分: ${this.score}，是否重新开始?`,
